    ...
    public void vendreActions(Action action, int quantity) {
        if (!listeActions.containsKey(action)) {
            throw new IllegalArgumentException(
                    "Vous essayez de vendre une action qui n'est pas présente dans votre portefeuille.");
        }
        if (listeActions.get(action) < quantity) {
            throw new IllegalArgumentException(
                    "Vous ne pouvez pas vendre plus d'action que ce que vous avez dans votre portefeuille");
        }
        if (listeActions.get(action) == quantity) {
